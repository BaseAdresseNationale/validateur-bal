...
function printReport(report) {
  const {fileValidation, parseErrors, notFoundFields, unknownFields, aliasedFields, rowsWithWarningsOrErrors, validatedRows} = report
  ...
  console.log('')
  if (rowsWithWarningsOrErrors && rowsWithWarningsOrErrors.length > 0) {
    rowsWithWarningsOrErrors.forEach(row => {
      if (row._warnings) {
