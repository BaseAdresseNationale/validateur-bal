...
function printReport(report) {
  const {parseMeta, parseErrors, notFoundFields, unknownFields, aliasedFields, rowsWithErrors, validatedRows} = report
  const {encoding, linebreak, delimiter} = parseMeta
  ...
  console.log(`Séparateur de ligne : ${humanizeLinebreak(linebreak)} => ${['\n', '\r\n'].includes(linebreak) ? 'OK' : 'Pas OK'}`)
  console.log(`Séparateur de colonne : ${delimiter} => ${delimiter === ';' ? 'OK' : 'Pas OK'}`)
  ...
  if (rowsWithErrors) {
    rowsWithErrors.forEach(row => {
      if (row.errors) {
        row.errors.forEach(err => console.log(`${row.cle_interop.rawValue} ${err}`))
      }
    })
    console.log('')
  }
