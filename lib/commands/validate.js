...
const {validate, getValidationErrorLabel, getValidationErrorSeverity} = require('..')
...
function printReport(report) {
  const {fileValidation, parseOk, parseErrors, notFoundFields, fields, rows, profilesValidation} = report
  ...
  if (rowsWithIssues.length > 0) {
    rowsWithIssues.forEach(row => {
      if (row._errors) {
        row._errors.forEach(err => console.log(`[${printSeverity(getValidationErrorSeverity(err))}] ${row.cle_interop.rawValue} ${getValidationErrorLabel(err)}`))
      }
    })
    console.log('')
  }
  ...
  Object.values(profilesValidation).forEach(profileValidation => {
