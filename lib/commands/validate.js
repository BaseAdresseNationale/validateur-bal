...
const {validate, getLabel, getValidationErrorSeverity} = require('..')
...
function printReport(report) {
  ...
  if (rowsWithIssues.length > 0) {
    rowsWithIssues.forEach(row => {
      if (row._errors) {
        row._errors.forEach(err => console.log(`[${printSeverity(getValidationErrorSeverity(err.code))}] ${row.cle_interop.rawValue} ${getLabel(err.code)}`))
      }
    })
    console.log('')
  }
