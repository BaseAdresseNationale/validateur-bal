...
class Validator {
  async validate(file) {
    ...
    return {
      ...
      counts: this.counts,
      rowsWithWarningsOrErrors: this.rowsWithWarningsOrErrors,
      validRows: this.validRows,
      acceptableRows: this.acceptableRows,
      normalizedRows: this.normalizedRows,
      validatedRows: this.validatedRows,
      rowsErrorsCount: this.rowsErrorsCount,
      rowsWarningsCount: this.rowsWarningsCount,
  ...
  async parseBlob(blob) {
    ...
    const parseResult = await parseCSV(blob, {skipEmptyLines: true, encoding: blobEncoding, header: true})
  ...
  validateRows() {
    ...
    this.validatedRows = this.parsedRows.map((row, line) => {
      ...
      Object.keys(schema.fields).forEach(field => {
        ...
        if (def.required === 'error' && !rawValue) {
          ...
          if (def.enum.includes(rawValue.normalize())) {
            ...
            row[field] = {rawValue, warnings: [`La valeur '${rawValue}' du champ ${field} n’est pas autorisée`]}
          } else {
        ...
        if (row[field].warnings && row[field].warnings.length > 0) {
          this.rowsWarningsCount += row[field].warnings.length
      ...
      this.rowsErrorsCount += rowLevelResult.errors.length
      this.rowsWarningsCount += rowLevelResult.warnings.length
    ...
    this.rowsWithWarningsOrErrors = this.validatedRows.filter(row => (row._errors && row._errors.length > 0) || (row._warnings && row._warnings.length > 0))
    this.acceptableRows = this.validatedRows.filter(row => !row._errors || row._errors.length === 0)
    this.validRows = this.validatedRows.filter(row => (!row._errors || row._errors.length === 0) && (!row._warnings || row._warnings.length === 0))
    this.normalizedRows = this.acceptableRows.map(normalize)
  }
  ...
  validateGlobal() {
    ...
    this.isValid = encoding.isValid &&
      ...
      this.rowsErrorsCount === 0 &&
      this.rowsWarningsCount === 0 &&
      Object.keys(this.aliasedFields) === 0
  }
}
