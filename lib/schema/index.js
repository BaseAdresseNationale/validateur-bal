...
const {isCommune, isCommuneActuelle, isCommuneAncienne, getCommuneAncienne} = require('../cog')
...
exports.fields = {
  ...
  cle_interop: {
    ...
    parse(v, {addError, setAdditionnalValues}) {
      ...
      if (!isCommune(codeCommune.toUpperCase())) {
  ...
  commune_insee: {
    ...
    aliases: ['commune_in'],
    parse(v, {addError}) {
      if (!isCommuneActuelle(v.toUpperCase())) {
        addError('commune_invalide')
        return
      }
  ...
  commune_deleguee_insee: {
    ...
    trim: true,
    parse(v, {addError}) {
      if (!isCommuneAncienne(v.toUpperCase())) {
        addError('commune_invalide')
        return
      }
...
exports.row = (row, {addError}) => {
  ...
  if (row.parsedValues.commune_deleguee_insee && row.parsedValues.commune_insee) {
    ...
    const ancienneCommune = getCommuneAncienne(codeAncienneCommune)
