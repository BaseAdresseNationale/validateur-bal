...
function computeFields(originalFields, options) {
    ...
    function findField(field) {
      if (!foundFields.has(schemaName)) {
        ...
        if (candidate) {
          ...
          candidate.exactMatch = schemaName === field
...
function computeRows(parsedRows, {fields}) {
...
async function validate(file, options) {
  ...
  const {fields, notFoundFields} = computeFields(originalFields, options)
  const rows = computeRows(parsedRows, {fields})
