...
function getSchemaVersion(schemaField) {
  return schema.fields[schemaField].version || '1.1'
}
...
function computeFields(originalFields, {strict, uniqueErrors}) {
  ...
  return {
    ...
    notFoundFields: [...notFoundFields].map(schemaField => {
      const schemaVersion = getSchemaVersion(schemaField)
      return {schemaField, schemaVersion}
    })
  }
}
...
function validateGlobal({linebreak, encoding, delimiter, rows, fields, notFoundFields, uniqueErrors}) {
  ...
  if (!fileValidation.encoding.isValid) {
    uniqueErrors.add('file.encoding.mismatch')
  }

  if (!fileValidation.delimiter.isValid) {
    uniqueErrors.add('file.delimiter.mismatch')
  }

  if (!fileValidation.linebreak.isValid) {
    uniqueErrors.add('file.linebreak.mismatch')
  }
