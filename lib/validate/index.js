...
function validateGlobal({validatedRows, fileValidation, aliasedFields, notFoundFields}) {
  const {encoding, delimiter, linebreak} = fileValidation
  const isValid = encoding.isValid &&
    delimiter.isValid &&
    linebreak.isValid &&
    notFoundFields.length === 0 &&
    !validatedRows.some(row => row._errors && row._errors.length > 0) &&
...
async function validate(file, options) {
  ...
  const {validatedRows} = validateRows(parsedRows, {aliasedFields, unknownFields})
  const {isValid} = validateGlobal({validatedRows, fileValidation, aliasedFields, notFoundFields})
