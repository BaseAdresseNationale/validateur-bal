...
class Validator {
  ...
  validateRows() {
    ...
    this.validRows = this.validatedRows.filter(row => !row._errors || row._errors.length === 0)
  }
  ...
  collectData() {
    ...
    this.validatedRows.forEach(row => {
      ...
      if (row.cle_interop && row.cle_interop.more && row.cle_interop.more.codeCommune) {
        getCommune(row.cle_interop.more.codeCommune).voies.add(row.cle_interop.more.codeVoie)
      }
    })
    const codesFantoir = [...communes.keys()].reduce((acc, codeCommune) => {
      const commune = communes.get(codeCommune)
      commune.voies.forEach(codeVoie => acc.add(`${codeCommune}-${codeVoie}`))
      return acc
    }, new Set())
    this.counts = {
