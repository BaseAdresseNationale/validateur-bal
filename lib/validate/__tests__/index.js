...
test('validate a file with aliases / strict mode', async t => {
  const buffer = await readAsBuffer('aliases.csv')
  const report = await validate(buffer, {strict: true})
  t.is(Object.keys(report.aliasedFields).length, 0);
  [
    'voie_nom',
    'numero',
    'suffixe',
  ...
  t.is(report.knownFields.length, 5)
  t.is(report.notFoundFields.length, 9)
  t.is(report.unknownFields.length, 9)
})
