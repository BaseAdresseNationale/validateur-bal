...
exports.fields = {
  ...
  cle_interop: {
    ...
    parse: v => {
      ...
      return {errors, parsedValue: v, more: {codeCommune, codeRivoli, numeroVoie, suffixes}}
    }
  },
  ...
  numero: {required: true, parse: v => {
    ...
    if (parsedValue.toString() !== v) {
      errors.push('La valeur du champ numero doit être un entier. Valeur fournie : ' + v)
    }
  ...
  date_der_maj: {
    ...
    parse: v => {
      ...
      const parsedDate = DateTime.fromISO(v)
      if (!parsedDate.isValid) {
        res.errors.push('Date invalide')
        return res
      }
      res.parsedValue = parsedDate.toISODate()
      return res
    }
  }
...
exports.row = row => {
  ...
  if (row.cle_interop.parsedValue && row.numero.parsedValue) {
    ...
    if (row.numero.parsedValue.toString().padStart('0') !== numeroVoie) {
