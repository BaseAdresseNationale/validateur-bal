...
exports.fields = {
  ...
  voie_nom: {required: 'error', parse: v => {
    const r = initResult()
  ...
  numero: {required: 'error', aliases: ['nulmero'], parse: v => {
    const r = initResult()
  ...
  suffixe: {parse: v => {
    ...
    if (!trimmedValue.match(/^[\da-z]/i)) {
      r.errors.push('La valeur du champ suffixe doit commencer par un caractère alphanumérique.')
      return r
    }
  ...
  commune_insee: {
    ...
    parse: v => {
      ...
      if (trimmedValue !== v) {
        r.warnings.push('La valeur du champ commune_insee ne doit pas avoir d’espaces en début ou en fin de chaîne de caractère')
      }

      if (!codesCommunesActuelles.includes(trimmedValue.toUpperCase())) {
        r.errors.push('Le code INSEE de la commune n’est pas un code valide de commune actuelle.')
        return r
      }
  ...
  date_der_maj: {
    ...
    parse: v => {
      ...
      r.parsedValue = format(parsedDate, 'yyyy-MM-dd')
      return r
    }
  }
