...
const {trimStart, uniq} = require('lodash')
const communes = require('../minicog.json')

const codesCommunes = uniq(communes.map(c => c.code))
const communesAnciennes = uniq(communes.filter(c => c.chefLieu).map(c => c.code))
const codesCommunesActuelles = uniq(communes.filter(c => !c.chefLieu).map(c => c.code))
...
exports.fields = {
  ...
  cle_interop: {
    ...
    parse: (v, {addError, setAdditionnalValues}) => {
      ...
      if (!codesCommunes.includes(codeCommune.toUpperCase())) {
  ...
  commune_deleguee_insee: {
    ...
    parse: (v, {addError}) => {
      ...
      if (trimmedValue !== v) {
        addError('commune_deleguee_insee.espaces_debut_fin')
      }

      if (!communesAnciennes.includes(trimmedValue.toUpperCase())) {
